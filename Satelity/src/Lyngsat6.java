...
*/
        ...
        for (int i=0; i<tmp.size(); i++) {       
        	prop.setProperty("lyngsat."+(i+1)+".region", alString[i]);
        	prop.setProperty("lyngsat."+(i+1)+".url", alURL[i].toString());    	
        }
        try(FileWriter output = new FileWriter("props.properties", Charset.forName("UTF-8"))){
        	prop.store(output, "These are properties");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
